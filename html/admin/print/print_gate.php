...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($filter_type) {
        case 'weekly':
            $query .= " WHERE YEARWEEK(eag.entry_time, 1) = YEARWEEK(?, 1)";
            break;
        case 'monthly':
            $query .= " WHERE YEAR(eag.entry_time) = YEAR(?) AND MONTH(eag.entry_time) = MONTH(?)";
            break;
        case 'yearly':
            $query .= " WHERE YEAR(eag.entry_time) = YEAR(?)";
            break;
        default: // daily
            $query .= " WHERE DATE(eag.entry_time) = ?";
            break;
    }

    // Add first name and last name filters if provided
    if (!empty($first_name_filter)) {
        $query .= " AND er.first_name LIKE ?";
    }
    if (!empty($last_name_filter)) {
        $query .= " AND er.last_name LIKE ?";
    }
    ...
    // Bind parameters for first name and last name filters
    if (!empty($first_name_filter)) {
        $param_types .= "s";
