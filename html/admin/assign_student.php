...
if ($search) {
    // Split the search term into components (e.g., "BSMB 4 A")
    $search_parts = explode(" ", $search);
    $department_search = isset($search_parts[0]) ? $search_parts[0] : '';
    $year_level_search = isset($search_parts[1]) ? $search_parts[1] : '';
    $section_search = isset($search_parts[2]) ? $search_parts[2] : '';

    // Modify SQL query to include department, year level, and section
    $sql = "SELECT * FROM student_registration WHERE 
    ...
    $search_term = "%$search%";
    $department_term = "%$department_search%";
    $year_level_term = "%$year_level_search%";
    $section_term = "%$section_search%";

    $stmt->bind_param("ssssss", $search_term, $search_term, $search_term, $department_term, $year_level_term, $section_term);
    $stmt->execute();
                        ...
                        <div class="row mb-3">
                            <div class="col-lg-12">
                                <form class="d-flex" method="GET" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                    <input class="form-control me-2" type="search" id="searchInput" placeholder="Search by Department, Year Level, Section" aria-label="Search" name="search" value="<?php echo htmlspecialchars($search); ?>">
                                </form>
                            </div>
                        </div>
                        <div class="row">
                                                ...
                                                <tbody id="studentTableBody">
                                                    ...
                                                    if ($result->num_rows > 0) {
                                                        while ($row = $result->fetch_assoc()) {
                                                            ...
                                                            echo "<td class='text-nowrap'>{$row['year_level']}</td>";
                                                            echo "</tr>";
                                                            $counter++;
                                                        }
                                                    } else {
                                                        echo "<tr><td colspan='7'>No students found for the given criteria.</td></tr>";
                                                    }
                                                    ?>
                                                </tbody>
                                            </table>
